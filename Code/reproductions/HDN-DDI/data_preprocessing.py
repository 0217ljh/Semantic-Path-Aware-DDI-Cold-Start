"""HDN-DDI: byte-exact port of jcsun-00/HDN-DDI/drugbank_test/data_preprocessing.py

Original source: https://github.com/jcsun-00/HDN-DDI/blob/main/drugbank_test/data_preprocessing.py

Changes from the original (kept minimal):
    1. File paths point to ``_Original-Dataset/`` (sibling folder of this
       module) rather than the hard-coded ``drugbank_test/DrugBank/...``
       from jcsun-00's repo layout. Naming follows CLAUDE.md §"复现代码
       规范" §1 (underscore prefix on project-metadata folders).
    2. The hierarchical mol-graph pkl is loaded via ``_pkl_loader``
       (detect → load, prefer ``__official``, fall back to ``__mine``)
       instead of a bare ``pkl.load(open(...))``, with a helpful
       FileNotFoundError + builder-CLI hint when both candidates are
       missing. Reproduction pipeline does NOT auto-build (per §复现
       规范 step 6 — auto-build is baseline-side territory).

Everything else (DrugDataset, negative-sampling rules, BipartiteData,
get_bipartite_graph, MOL_EDGE_LIST_FEAT_MTX, ALL_TRUE_H_WITH_TR /
ALL_TRUE_T_WITH_HR / FREQ_REL bookkeeping) is byte-exact to the upstream.
"""
...
from _pkl_loader import ensure_id_data_dict_pkl


# ---------------------------------------------------------------------
# Path adjustment (deviation #1 from upstream):
# upstream hard-codes ``drugbank_test/DrugBank/...``; we point at the
# sibling ``_Original-Dataset/`` cloned from jcsun-00/DrugBank. Naming
# follows CLAUDE.md §"复现代码 (Reproduction) 规范" §1 (underscore
# prefix on project-metadata folders).
# ---------------------------------------------------------------------
_DATA_DIR = Path(__file__).resolve().parent / "_Original-Dataset"

# Deviation #2: route pkl load through the loader so missing official
# pkl falls back to our reproduction-side __mine.pkl, with a clear error
# (NOT auto-build) when both candidates are missing.
# Loader prints one stderr line identifying which file fed this run.
id_data_dict = ensure_id_data_dict_pkl(
    _DATA_DIR / "necessary",
    upstream_smiles_csv=_DATA_DIR / "drug_smiles.csv",
)

df_drugs_smiles = pd.read_csv(str(_DATA_DIR / "drug_smiles.csv"))
...
def get_mol_edge_list_and_feat_mtx(mol_graph):
    ...
    n_features.sort()  # align feature matrix to atom indices
    ...
    undirected_edge_list = (
        torch.cat([edge_list, edge_list[:, [1, 0]]], dim=0).T
        if len(edge_list)
        else edge_list
    )
...
df_all_pos_ddi = pd.read_csv(str(_DATA_DIR / "ddis.csv"))
all_pos_tup = [
    (h, t, r)
    for h, t, r in zip(df_all_pos_ddi["d1"], df_all_pos_ddi["d2"], df_all_pos_ddi["type"])
]
...
class DrugDataset(Dataset):
    def __init__(self, tri_list, ratio=1.0, neg_ent=1, disjoint_split=True, shuffle=True):
        """disjoint_split: whether entities should appear in one and only
    ...
    def __corrupt_ent(self, other_ent, r, other_ent_with_r_dict, max_num=1):
        ...
        while current_size < max_num:
            candidates = np.random.choice(
                self.drug_ids, (max_num - current_size) * 2
            )
            mask = np.isin(
                candidates,
