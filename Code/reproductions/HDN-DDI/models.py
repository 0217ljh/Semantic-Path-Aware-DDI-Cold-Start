"""HDN-DDI: byte-exact port of jcsun-00/HDN-DDI/drugbank_test/models.py

Original source: https://github.com/jcsun-00/HDN-DDI/blob/main/drugbank_test/models.py
No semantic changes — only re-saved with this docstring header and the
import path changed from ``from layers import ...`` to a relative import.
"""
...
class HDN_DDI(nn.Module):
    def __init__(
        self, in_features, hidd_dim, kge_dim, rel_total,
        ...
        self.initial_conv = GATConv(
            self.in_features, heads_out_feat_params[0], blocks_params[0]
        )
...
class HDN_DDI_Block(nn.Module):
    ...
    def forward(self, h_data, t_data, b_graph):
        ...
        h_data.x, h_weight = self.pool(
            h_data.x, h_data.edge_index, return_attention_weights=True
        )
        t_data.x, t_weight = self.pool(
            t_data.x, t_data.edge_index, return_attention_weights=True
        )
