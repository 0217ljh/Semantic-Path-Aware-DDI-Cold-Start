...
class RGCNBasisLayer(RGCNLayer):
    ...
    def propagate(self, g, attn_rel_emb = None, nonKG = True):
        ...
        # DGL-2.4 port: in DGL 2.x update_all does NOT invoke the reduce for nodes
        # with no incoming edges, so those nodes get no 'h' (DGL 0.4 wrote every
        # node). After removing the root-root edge, small enclosing subgraphs can have
        # isolated nodes -> downstream g.ndata['h'] KeyError. Pre-init 'h' to zeros so
        # isolated nodes keep a zero embedding; message-receiving nodes are overwritten.
        g.ndata['h'] = torch.zeros(g.number_of_nodes(), self.out_dim, device=weight.device)
