"""Run NodeDup-augmented EmerGNN S2-only."""
...
from my_code.models.screen_s2_iter.nodedup_trainer import _PerModeEmerGNN_NodeDup  # noqa
...
def main():
    ...
    p.add_argument("--ndup-lambda", type=float, default=0.3)
    p.add_argument("--ndup-all-drugs", action="store_true",
                   help="If set, duplicate ALL drugs, not just low-degree.")
    p.add_argument("--tag", type=str, default=None)
    args = p.parse_args()

    tag = args.tag or f"nodedup_lam{args.ndup_lambda}_{'all' if args.ndup_all_drugs else 'lowdeg'}_{args.backbone_kg_source}_seed{args.seed}"
    with RunLogger(script="run_nodedup", tag=tag, seed=args.seed) as rl:
        rl.set_meta(
            kg_source=args.backbone_kg_source, epochs=args.epochs, seed=args.seed,
            ndup_lambda=args.ndup_lambda, ndup_all_drugs=args.ndup_all_drugs,
        )
        print(f"[ndup] config: {vars(args)}")
        ...
        kwargs = dict(
            ...
            ndup_anchor_lambda=args.ndup_lambda,
            ndup_low_degree_only=(not args.ndup_all_drugs),
        )
        trainer = _PerModeEmerGNN_NodeDup(**kwargs)
        t0 = time.time()
        trainer.fit(ds, val=ds)
        fit_sec = time.time() - t0
        m = _eval_s2(trainer, ds)
        print(f"[ndup] test_s2: AUC={m['auc']:.4f} NLL={m['nll']:.4f}, fit={fit_sec/3600:.2f}h")
        payload = {
