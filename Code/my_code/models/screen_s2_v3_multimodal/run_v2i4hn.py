"""Run E-i4-HN: MNAH + I4 + hard-negative mining (codex 019e76f6 next experiment).
Primary comparison: MNAH 0.772 vs v2i4 0.7804 vs v2i4hn ?
"""
...
from my_code.models.screen_s2_v3_multimodal.v2i4hn_trainer import _PerModeEmerGNN_V2I4HN  # noqa
...
def _eval_s2(model, ds):
    ...
    try:
        ...
        print(f"[v2i4hn] channel diag failed: {exc}", flush=True)
...
def main():
    ...
    p.add_argument("--hn-frac", type=float, default=0.5)
    p.add_argument("--hn-warmup-epochs", type=int, default=5)
    p.add_argument("--v2i4-shuffle-control", action="store_true")
    p.add_argument("--tag", type=str, default=None)
    args = p.parse_args()

    tag = args.tag or f"v2i4hn_seed{args.seed}"
    with RunLogger(script="run_v2i4hn", tag=tag, seed=args.seed) as rl:
        rl.set_meta(epochs=args.epochs, seed=args.seed, hn_frac=args.hn_frac,
                    hn_warmup=args.hn_warmup_epochs, shuffle_control=args.v2i4_shuffle_control)
        print(f"[v2i4hn] config: {vars(args)}")
        from data_utils import PairDataset
        ds = PairDataset.from_pkl(str(PKL))
        trainer = _PerModeEmerGNN_V2I4HN(
        ...
        print(f"[v2i4hn] run_dir: {rl.run_dir}")
