"""EmerGNN per-mode training helper (INTERNAL, not registered as a baseline).

This file holds :class:`_PerModeEmerGNN`, the single-mode EmerGNN training
implementation. It is NOT a standalone public baseline — it is a building
block used by:

  * :class:`baseline.emergnn.binary_cls.baseline.EmerGNNBaseline` — the
    public binary baseline, a multimode wrapper that internally trains
    3 instances of ``_PerModeEmerGNN`` (one per S0/S1/S2 mode, matching
    upstream paper convention). This is the new default registered as
    ``"emergnn"`` (2026-05-20 promotion).
  * :class:`baseline.emergnn.multi_cls.baseline.EmerGNNMulticlassBaseline`
    — the K-way multi-class variant that inherits from this class.
  * :class:`baseline.emergnn.deprecate.binary_cls_kgonly.baseline.EmerGNNKGOnlyBaseline`
    — the kgonly Method B variant kept under deprecate/ as a reference
    experiment (companion to the multimode wrapper that won the 5/20
    comparison).
...
NOTE — paper-vs-port task formulation:
  * Original paper (Zhang et al., Nat Comp Sci 2023) on DrugBank does
    **multi-class** (predict-which-of-86-types, softmax margin loss,
    ...
    paper; see ``multi_cls/baseline.py`` for that variant.
  * THIS class is a per-mode binary adapter — single ``shuffle_train_mode``
    + single training run. To evaluate paper-faithfully on all of
    S0/S1/S2 in our project setting use the multimode wrapper at
    ``baseline/emergnn/binary_cls/baseline.py`` (registered as ``"emergnn"``),
    which orchestrates 3 of these per-mode instances with the right
    upstream-matched hyperparams per setting.
...
from baseline.base import BaselineModel, write_manifest
...
# NOT registered: this is an internal helper, not a standalone baseline.
# The public ``"emergnn"`` registry slot is taken by the multimode wrapper at
# ``baseline/emergnn/binary_cls/baseline.py`` which orchestrates 3 instances
# of this class (one per S0/S1/S2 mode).
class _PerModeEmerGNN(BaselineModel):
    ...
    def _pair_indices(self, pairs: pd.DataFrame) -> tuple[torch.Tensor, torch.Tensor]:
        if self._entity2id is None:
            raise RuntimeError("_PerModeEmerGNN.fit() must be called before predict_proba.")
    ...
    def predict_proba(
        ...
        *,
        ...
        if self._model is None:
            raise RuntimeError("_PerModeEmerGNN must be fitted (or loaded) before prediction.")
    ...
    @classmethod
    def load(cls, path: "Path | str") -> "_PerModeEmerGNN":
        p = Path(path)
