...
Semantics per ``setting`` prefix (matches paper code lines 144-178):
  * S0: random 80/20 split of all train_ddi (RE-shuffled every epoch)
  * S1: target = exactly one endpoint in kept (one-old + one-new)
  * S2: target = both endpoints removed (two-new)
  * FIXED: like S0 but a SINGLE deterministic 80/20 split reused every epoch
    (no per-epoch reshuffle -> a stable, leakage-free training graph for the
    rank-transfer diagnosis). An optional trailing integer selects the split
    seed for multi-seed robustness runs (``FIXED`` -> seed 0, ``FIXED1`` ->
    seed 1, ...). The passed ``rng`` is intentionally ignored for the split so
    it never drifts across epochs.
"""
...
def shuffle_train(
    ...
    *,
    ...
    # ── FIXED: S0-style 80/20 split but a SINGLE deterministic realization
    #    reused every epoch (no per-epoch reshuffle). Optional trailing int in
    #    the setting string selects the split seed ("FIXED"->0, "FIXED1"->1).
    #    The passed ``rng`` is IGNORED for the split so it stays identical
    #    across epochs (stable, leakage-free training graph for diagnosis).
    if setting.startswith("FIXED"):
        n_all = len(train_ddi)
        if n_all == 0:
            return train_kg.copy(), np.zeros((0, 3), dtype=np.int64)
        suffix = setting[len("FIXED"):]
        split_seed = int(suffix) if suffix.isdigit() else 0
        fixed_rng = np.random.default_rng(split_seed)
        perm = fixed_rng.permutation(n_all)
