...
def links2subgraphs(A, graphs, params, max_label_value=None):
    ...
    # lmdb-2.x port: map_size must be an int (older lmdb accepted floats). Also add a
    # generous headroom factor + a 512 MB floor: our merged/TWOSIDES KGs yield larger
    # enclosing subgraphs than the original Hetionet data, so the 100-sample estimate
    # can underflow and raise MDB_MAP_FULL. (map_size is virtual address space, cheap.)
    map_size = int(map_size * 4) + (512 << 20)
