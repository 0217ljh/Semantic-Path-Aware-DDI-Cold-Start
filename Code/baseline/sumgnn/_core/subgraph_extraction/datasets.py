...
# lmdb-2.x port: lmdb 2.x refuses to open the same env twice in one process, but
# SumGNN opens the SAME db_path once per split (train/valid/test SubgraphDataset).
# Cache the read-only env per db_path and share it across instances (identical
# semantics; the original relied on lmdb 0.98 allowing duplicate opens).
_ENV_CACHE = {}


def _open_env(db_path):
...
class SubgraphDataset(Dataset):
    ...
    def _prepare_subgraphs(self, nodes, r_label, n_labels):
        ...
        # DGL-2.4 port: the original built a fresh DGLGraph from
        # ``self.graph.subgraph(nodes)`` and re-attached ``edata['type']`` via
        # ``.parent_eid``. ``dgl.node_subgraph`` induces the same node-relabeled
        # subgraph AND auto-inherits ``edata['type']`` (feature inheritance), with
        # the parent edge ids exposed as ``sg.edata[dgl.EID]``. Node order is
        # preserved (roots 0,1 stay at the front), matching the original.
        subgraph = dgl.node_subgraph(self.graph, nodes)
        subgraph.edata['type'] = self.graph.edata['type'][subgraph.edata[dgl.EID]]
        # DGL-2.4 port: node_subgraph auto-attaches reserved dgl.NID/dgl.EID node/edge
        # features. The original DGL-0.4 subgraph had none; drop them so downstream
        # batching + send_graph_to_device only carry the model's real features.
        ...
        # DGL-2.4 port: ``edge_id`` -> ``edge_ids``; raises DGLError when the edge
        # is absent, so guard on a membership check instead of AssertionError.
        ...
        # remove the direct edge(s) between the two roots so the model cannot
        # trivially read the DDI label off the subgraph (same intent as original).
        if subgraph.has_edges_between(0, 1):
            edges_btw_roots = subgraph.edge_ids(0, 1)
