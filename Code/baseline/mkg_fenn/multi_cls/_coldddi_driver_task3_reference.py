...
def read_dataset(drug_name_id,num):
    ...
    relations = {}
    drug_list=[]
    filename = "./Code and Datasets/dataset/dataset"+str(num)+".txt"
    filename_my = "./my_data/new_data/dataset"+str(num)+".txt"
...
def main():
    conn = sqlite3.connect("./Code and Datasets/dataset/event.db")
    df_drug = pd.read_sql('select * from drug;', conn)
    extraction = pd.read_sql('select * from extraction;', conn)
    mechanism = extraction['mechanism']
    action = extraction['action']

    df_drug_my = pd.read_csv('/mnt/e/Code/My_project/DDI_Benchmark_Pipeline/Dataset/Drugbank/Set_6/drugs.csv')
    extraction_my = pd.read_csv('/mnt/e/Code/My_project/DDI_Benchmark_Pipeline/Dataset/Drugbank/Set_6/ddi_edges.csv')
    id2name_my = dict(zip(df_drug_my["drugbank_id"].astype(str), df_drug_my["name"].astype(str)))
    # 将extraction_my中的drug_a_id和drug_b_id转换为name,加两列进去，分别名为drugA和drugB
    extraction_my['drugA'] = extraction_my['drug_a_id'].map(id2name_my)
    extraction_my['drugB'] = extraction_my['drug_b_id'].map(id2name_my)

    mechanism_my = extraction_my['ddi_type']
    # action_my就是将mechanism_my中的每个元素换成'',保持相同的type(mechanism_my)
    action_my = ['' for _ in range(len(mechanism_my))]
    action_my = type(mechanism_my)(action_my)

    drugA = extraction_my['drugA']
    drugB = extraction_my['drugB']
    new_label,account_num = prepare(mechanism_my, action_my)
    new_label = np.array(new_label)

    dict1 = {}
    for i in df_drug_my["name"]:
        dict1[i] = len(dict1)
    drug_name = [dict1[i] for i in df_drug_my["name"]]
