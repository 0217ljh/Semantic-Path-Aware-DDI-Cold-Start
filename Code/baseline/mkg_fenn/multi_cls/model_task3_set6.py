...
class GNN1(nn.Module):
    def __init__(self,dataset,tail_len,relation_len,args,dict1,drug_name,**kwargs):
        ...
        self.drug_embed = nn.Embedding(num_embeddings=1994, embedding_dim=args.embedding_num)
        ...
        self.W1 = nn.Parameter(torch.randn(size=(1994,args.embedding_num,args.embedding_num)))
        self.b1 = nn.Parameter(torch.randn(size=(args.neighbor_sample_size,args.embedding_num)))
        self.W2 = nn.Parameter(torch.randn(size=(1994, args.embedding_num, args.embedding_num)))
    ...
    def forward(self,datas):
        ...
        drug_rel = drug_embedding.reshape((1994, 1, args.embedding_num)) * rela_embedding
        ...
        weighted_ent = drug_rel_score.reshape((1994, 1, args.neighbor_sample_size)).matmul(ent_embedding)
        drug_e = torch.cat([weighted_ent.reshape(1994, args.embedding_num), drug_embedding.reshape((1994, args.embedding_num))], dim=1)
    ...
    def arrge(self,kg,drug_name_id,neighbor_sample_size,n_drug=1994):
...
class GNN3(nn.Module):
    def __init__(self,dataset,tail_len,relation_len,args,dict1,drug_name,**kwargs):
        ...
        self.ent_embed = nn.Embedding(num_embeddings=1994, embedding_dim=args.embedding_num)
        self.W1 = nn.Parameter(torch.randn(size=(1994,args.embedding_num,args.embedding_num)))
        self.b1 = nn.Parameter(torch.randn(size=(args.neighbor_sample_size,args.embedding_num)))
        self.W2 = nn.Parameter(torch.randn(size=(1994, args.embedding_num, args.embedding_num)))
        self.b2 = nn.Parameter(torch.randn(size=(args.neighbor_sample_size, args.embedding_num)))
    ...
    def arrge(self,kg,drug_name_id,neighbor_sample_size,n_drug=1994,tails_num=570,relations_num=65):
        drug_number = []
        drug_list=[]
        for i in drug_name_id:
