...
from model_task3 import FusionLayer,GNN1,GNN2,GNN3,GNN4
...
# --- event.db repoint: run the (adapted, ghost-node) MKG-FENN task3 on the
#     UPSTREAM original 65-event dataset (event.db), both-unseen / multi-cls.
...
OUT_DIR = str(_Path(__file__).resolve().parent / "_mkgfenn_task3_out")

parser = argparse.ArgumentParser(description='GNN based on the whole datas')
parser.add_argument("--epoches",type=int,choices=[10,100,500,1000,2000],default=120)
parser.add_argument("--batch_size",type=int,choices=[102400,2048,1024,512,256,128],default=1024)
...
parser.add_argument("--event_num",type=int,default=215)

parser.add_argument("--n_drug",type=int,default=1994)
...
def read_dataset(drug_name_id,num):
    ...
    relations = {}
    drug_list=[]
    filename_my = DATASET_DIR + "/dataset" + str(num) + "__official.txt"
    with open(filename_my, encoding="utf8") as reader:
        for line in reader:
...
def train(train_x,train_y,test_x,test_y,net,test_adj):
    ...
    test_list = []
    max_test_output = torch.zeros((0,args.event_num),dtype=torch.float)
...
def find_dif(raw_matrix):
    ...
    #                 else:
    #                     sim_matrix4[i, j] += 0
    X = np.asarray(raw_matrix)          # (dim, 20)
    sim = (X[:, None, :] == X[None, :, :]).sum(axis=2).astype(np.float32)  # (dim, dim), 每对相等的维度数(0~20)
    np.fill_diagonal(sim, 0)            # i==j 置 0
    return sim
    #return sim_matrix4
...
def main():
    # event.db native loading (upstream 65-event DDIMDL dataset).
    ...
    args.event_num = event_num   # 65 for event.db (overrides CLI default)
    ...
    feature_matrix1 = np.zeros((len(dict1), dataset1_tail_len), dtype=float)
    feature_matrix2 = np.zeros((len(dict1), dataset2_tail_len), dtype=float)
    feature_matrix3 = np.zeros((len(dict1), len(dict1)), dtype=float)
    feature_matrix4 = np.zeros((len(dict1), dataset4_tail_len), dtype=float)
    ...
    drug_sim3 = Jaccard(feature_matrix3)

    drug_sim4 = find_dif(feature_matrix4)


    temp_drugA = [[] for i in range(args.event_num)]
    temp_drugB = [[] for i in range(args.event_num)]
    for i in range(len(new_label)):
        temp_drugA[new_label[i]].append(drugA_id[i])
        temp_drugB[new_label[i]].append(drugB_id[i])

    drug_cro_dict = {}
    for i in range(args.event_num):
    ...
    for cross_ver in range(5):
        ...
        drugA_id_np = np.asarray(drugA_id)
        drugB_id_np = np.asarray(drugB_id)

        train_set = np.array(train_drug[cross_ver])

        a_in = np.isin(drugA_id_np, train_set)
        b_in = np.isin(drugB_id_np, train_set)

        train_mask = a_in & b_in
        test_mask  = (~a_in) & (~b_in)

        X_train = np.flatnonzero(train_mask).tolist()
        y_train = X_train

        X_test  = np.flatnonzero(test_mask).tolist()
        y_test  = X_test
    ...
    save_result(OUT_DIR + "/", "all", result_all)
    save_result(OUT_DIR + "/", "each", result_eve)
    metrics = {
        "ACC": float(result_all[0]), "micro_AUPR": float(result_all[1]),
        "micro_AUC": float(result_all[3]), "macro_F1": float(result_all[6]),
        "macro_PRE": float(result_all[8]), "macro_REC": float(result_all[10]),
        "event_num": int(args.event_num), "n_drug": int(len(dict1)),
        "n_pairs": int(len(new_label)), "n_test_pairs": int(len(y_true)),
    }
    with open(OUT_DIR + "/metrics.json", "w") as _f:
        _json.dump(metrics, _f, indent=2)
    print("=== MKG-FENN task3 (both-unseen, event.db 65-event) ===", flush=True)
    print("  ACC=%.4f micro-AUPR=%.4f micro-AUC=%.4f macro-F1=%.4f macro-PRE=%.4f macro-REC=%.4f"
          % (result_all[0], result_all[1], result_all[3], result_all[6],
             result_all[8], result_all[10]), flush=True)
    print("  reference Table 4 (paper, task3): ACC=0.4552 AUC=0.9149 F1=0.2186", flush=True)
    print('%d-fold validation: avg train acc  %f, avg test acc %f' % (5, train_sum / 5, test_sum / 5), flush=True)
    return
