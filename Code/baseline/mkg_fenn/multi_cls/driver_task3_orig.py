...
from model_task3_orig import FusionLayer,GNN1,GNN2,GNN3,GNN4
...
def main():
    ...
    metrics = {
        ...
        "event_num": int(args.event_num), "n_pairs": int(len(y_true)),
    }
    with open(OUT_DIR + "/metrics.json", "w") as _f:
        _json.dump(metrics, _f, indent=2)
    print("=== ORIG MKG-FENN task3 (both-unseen, event.db 65-event) ===", flush=True)
    ...
    print("  reference Table 4 (paper, task3): ACC=0.4552 AUPR=0.4162 AUC=0.9149 F1=0.2186 PRE=0.2754 REC=0.2131", flush=True)
