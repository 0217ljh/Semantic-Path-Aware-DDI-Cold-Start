...
class GNN1(nn.Module):
    def __init__(self,dataset,tail_len,relation_len,args,dict1,drug_name,**kwargs):
        ...
        self.drug_embed = nn.Embedding(num_embeddings=len(dict1), embedding_dim=args.embedding_num)
        ...
        self.rela_embed = nn.Embedding(num_embeddings=relation_len["dataset1"] + 1, embedding_dim=args.embedding_num)
        self.ent_embed = nn.Embedding(num_embeddings=tail_len["dataset1"] + 1, embedding_dim=args.embedding_num)
        # 设置初始的ghost node的embedding 为0
        with torch.no_grad():
            self.ent_embed.weight[self.ghost_ent_id].zero_()
            self.rela_embed.weight[self.ghost_rel_id].zero_()
        self.W1 = nn.Parameter(torch.randn(size=(len(dict1),args.embedding_num,args.embedding_num)))
        self.b1 = nn.Parameter(torch.randn(size=(args.neighbor_sample_size,args.embedding_num)))
        self.W2 = nn.Parameter(torch.randn(size=(len(dict1), args.embedding_num, args.embedding_num)))
    ...
    def forward(self,datas):
        ...
        adj_tail,adj_relation=self.arrge(kg,dict1,args.neighbor_sample_size,n_drug=len(self.dict1))
        ...
        drug_rel = drug_embedding.reshape((len(self.dict1), 1, args.embedding_num)) * rela_embedding
        ...
        weighted_ent = drug_rel_score.reshape((len(self.dict1), 1, args.neighbor_sample_size)).matmul(ent_embedding)
        drug_e = torch.cat([weighted_ent.reshape(len(self.dict1), args.embedding_num), drug_embedding.reshape((len(self.dict1), args.embedding_num))], dim=1)
    ...
    def arrge(self,kg,drug_name_id,neighbor_sample_size,n_drug=572):
        ...
        for i in drug_name_id:
            ...
            if n_neighbor == 0:
                adj_tail[drug_name_id[i]] = np.array([self.ghost_ent_id])
                adj_relation[drug_name_id[i]] = np.array([self.ghost_rel_id])
                continue
...
class GNN2(nn.Module):
    def __init__(self,dataset,tail_len,relation_len,args,dict1,drug_name,**kwargs):
        ...
        self.drug_name, self.args = drug_name, args
        self.drug_embed = nn.Embedding(num_embeddings=len(self.dict1), embedding_dim=args.embedding_num)
        ...
        self.W1 = nn.Parameter(torch.randn(size=(len(self.dict1),args.embedding_num,args.embedding_num)))
        self.b1 = nn.Parameter(torch.randn(size=(args.neighbor_sample_size,args.embedding_num)))
        self.W2 = nn.Parameter(torch.randn(size=(len(self.dict1), args.embedding_num, args.embedding_num)))
        self.b2 = nn.Parameter(torch.randn(size=(args.neighbor_sample_size, args.embedding_num)))
        self.Linear1 = nn.Sequential(nn.Linear(args.embedding_num * 2, args.embedding_num),
                                     nn.ReLU(),
                                     nn.BatchNorm1d(args.embedding_num))
...
class GNN3(nn.Module):
    def __init__(self,dataset,tail_len,relation_len,args,dict1,drug_name,**kwargs):
        ...
        self.ent_embed = nn.Embedding(num_embeddings=len(self.dict1), embedding_dim=args.embedding_num)
        self.W1 = nn.Parameter(torch.randn(size=(len(self.dict1),args.embedding_num,args.embedding_num)))
        self.b1 = nn.Parameter(torch.randn(size=(args.neighbor_sample_size,args.embedding_num)))
        self.W2 = nn.Parameter(torch.randn(size=(len(self.dict1), args.embedding_num, args.embedding_num)))
        self.b2 = nn.Parameter(torch.randn(size=(args.neighbor_sample_size, args.embedding_num)))
    ...
    def forward(self,arguments):
        ...
        adj_tail,adj_relation=self.arrge(kg,dict1,args.neighbor_sample_size,n_drug=len(self.dict1),tails_num=self.ent_embed.num_embeddings,relations_num=self.rela_embed.num_embeddings)
        drug_name=torch.LongTensor(drug_name)
        adj_tail=torch.LongTensor(adj_tail)
        adj_relation=torch.LongTensor(adj_relation)
        drug_embedding = self.drug_embed(drug_name)
