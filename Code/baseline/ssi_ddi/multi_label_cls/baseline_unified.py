"""SSI-DDI MULTILABEL baseline for the UNIFIED benchmark (Case-B).
...
SSI-DDI multilabel model (:class:`baseline.ssi_ddi.multi_label_cls.baseline.
SSIDDIMultilabelBaseline`) reusing the paper's ALGORITHM CORE UNCHANGED
(substructure-aware GAT blocks + co-attention + RESCAL all-relation head).
Case-B adaptation: only the head activation / loss / targets / metric are
swapped. NO cross-regime routing.

SSI-DDI is KG-FREE / molecular — no merged-KG path is required; it builds its
per-drug PyG graphs from ``drugs[[drugbank_id, smiles]]`` (TWOSIDES ships SMILES
for all 604 drugs). The only bridge to the unified benchmark is the DDI pairs,
delivered as a paired pos/neg 200-multihot bundle via
...
Independence: imports ONLY ``baseline.ssi_ddi.*`` + ``data_utils`` (per
CLAUDE.md §"文件级独立性"); the HDN-DDI / EmerGNN multilabel wrappers were READ
as reference patterns but are not imported.
"""
...
from baseline.ssi_ddi.multi_label_cls.baseline import SSIDDIMultilabelBaseline  # noqa: E402
...
class SSIDDIUnifiedMultilabel(UnifiedBaseline):
    task = "multilabel"

    def __init__(self, *, n_epochs: int = 5, batch_size: int = 256,
        ...
        self._core: SSIDDIMultilabelBaseline | None = None
    ...
    def _locate_pair_links(self, resources: "LeafResources", fold: str):
        ...
        — mirrors the HDN-DDI / EmerGNN multilabel wrappers)."""
        meta = resources.meta
    ...
    def fit(self, train_df: "pd.DataFrame", val_df: "pd.DataFrame | None" = None, *,
        ...
        # ``ds`` only feeds the inherited ``_build_graphs`` (SSI-DDI per-drug PyG
        # graphs from drugs[[drugbank_id, smiles]]); the actual training targets
        ...
        self._core = SSIDDIMultilabelBaseline(
...
__all__ = ["SSIDDIUnifiedMultilabel"]
