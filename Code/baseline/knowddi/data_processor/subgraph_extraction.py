"""KnowDDI's ``data_processor/subgraph_extraction.py`` — enclosing subgraph + LMDB.

Faithful port of KnowDDI's official
``Paper/Reference/Original-Code/KnowDDI/pytorch/data_processor/subgraph_extraction.py``.
This extracts the h-hop ENCLOSING subgraph around each DDI link (double-radius node
labeling, :115 ``node_label`` / :138 ``subgraph_extraction_labeling``) and writes the
pruned node lists + labels into per-split LMDB dbs. Divergence-point 1 (directional
pruning ``extract_r_digraph``) happens LATER at load time in ``datasets.py``; this
module produces the enclosing-subgraph node set the paper's algorithm prunes.

No DGL API here (all numpy / scipy / lmdb / multiprocessing), so this is a
byte-faithful port modulo ``from __future__`` + docstring. INDEPENDENT copy
(CLAUDE.md file-independence); imports only from ``baseline.knowddi.utils``.
"""
...
from ..utils.graph_utils import _bfs_relational, incidence_matrix, remove_nodes, serialize


def generate_subgraph_datasets(params, splits=None) -> None:
    if splits is None:
    ...
    if params.dataset == 'drugbank':
        ...
        adj_list, triplets, entity2id, relation2id, id2entity, id2relation, rel, triplets_mr, polarity_mr = \
            process_files_decagon(params.file_paths, BKG_file)
    ...
    for split_name in splits:
        if params.dataset == 'drugbank':
            ...
            files_data[split_name] = {'triplets': triplets_mr[split_name], 'max_size': params.max_links,
...
def links2subgraphs(adj_list, files_data, params, max_label_value=None) -> None:
    ...
    BYTES_PER_DATUM = get_average_subgraph_size(
        100, list(files_data.values())[0]['triplets'], adj_list, params) * 1.5
    links_length = 0
    for split_name, split in files_data.items():
    ...
    def extraction_helper(adj_list, links, g_labels, split_env):
        ...
        with mp.Pool(processes=params.num_workers if getattr(params, "num_workers", 0) else None,
                     initializer=intialize_worker, initargs=(adj_list, params, max_label_value)) as p:
...
def get_average_subgraph_size(sample_size, links, adj_list, params):
    ...
    for idx in lst:
        ...
        nodes, n_labels, subgraph_size, enc_ratio, num_pruned_nodes = subgraph_extraction_labeling(
            (n1, n2), r_label, adj_list, params.hop, params.enclosing_subgraph, params.max_nodes_per_hop)
...
def intialize_worker(adj_list, params, max_label_value) -> None:
...
def extract_save_subgraph(args_):
    ...
    pruned_subgraph_nodes, pruned_node_labels, subgraph_size, enc_ratio, num_pruned_nodes = \
        subgraph_extraction_labeling((n1, n2), r_label, adj_list_, params_.hop,
                                     params_.enclosing_subgraph, params_.max_nodes_per_hop)
    ...
    datum = {'nodes': pruned_subgraph_nodes, 'r_label': r_label, 'g_label': g_label,
             'n_labels': pruned_node_labels, 'subgraph_size': subgraph_size, 'enc_ratio': enc_ratio,
...
def node_label(subgraph, max_distance=1):
    ...
    dist_to_roots = [np.clip(ssp.csgraph.dijkstra(sg, indices=[0], directed=False, unweighted=True, limit=1e6)[:, 1:], 0, 1e7)
...
def subgraph_extraction_labeling(ind, rel, A_list, hop=1, enclosing_subgraph=False,
...
__all__ = ["generate_subgraph_datasets", "links2subgraphs", "subgraph_extraction_labeling"]
