"""Run PMP v1.5 cold-start DDI experiment (created 2026-06-03).

v1.5 = sequential cluster-attention -> within-pool with prior + content
residual attention (Codex Option B). Replaces v1.2's parallel concat fusion.

Step 1: alpha = softmax over (i, j) of
            log p_a[i] + log T[i, j] + log p_b[j]
            + lambda_content * g(H_a^i, H_b^j)
        with g_mlp's last layer zero-init and lambda_content init 0 so the
        model starts at pure affinity routing and learns content-conditional
        attention.

Step 2: cluster-attention-marginal weighted within-pool over intersection
        mediators.

Reuses PMP cache v1 + v1.1 cluster cache v2.

Example:
  python Code/scripts/run_pmp_v1_5.py --epochs 100 --seed 42 --n-dim 32 \
      --tag pmp_v1_5_ndim32_seed42
"""
...
def _eval_s2(model, ds):
    ...
    # v1.5 lambda_content diagnostic
    try:
        lam = float(
            model._aux_mlp.lambda_content.detach().cpu().item()
        )
        out["lambda_content_final"] = lam
    except Exception as exc:
        print(f"[pmp-v1.5] lambda_content read failed: {exc}", flush=True)
...
def main():
    ...
    p.add_argument("--cluster-cache", type=str, default=str(DEFAULT_CLUSTER_CACHE))
    ...
    p.add_argument("--cluster-max-mediators-per-drug", type=int, default=64,
                   help="Cap; 0 disables (use full neighborhood).")
    p.add_argument("--content-hidden", type=int, default=16,
                   help="Hidden width of g_mlp content residual.")
    ...
    tag = args.tag or f"pmp_v1_5_seed{args.seed}"
    with RunLogger(script="run_pmp_v1_5", tag=tag, seed=args.seed) as rl:
        rl.set_meta(epochs=args.epochs, seed=args.seed)
        print(f"[pmp-v1.5] config: {vars(args)}")
        pkl_path = _pkl_for_seed(args.seed)
        print(f"[pmp-v1.5] using dataset: {pkl_path}")
        from data_utils import PairDataset
        ds = PairDataset.from_pkl(str(pkl_path))

        trainer = _PerModeEmerGNN_PMP_v1_5(
            n_dim=args.n_dim,
            length=args.length,
            feat="M",
            learning_rate=1e-3,
            ...
            content_hidden=args.content_hidden,
        )
        ...
        print(
            f"[pmp-v1.5] test_s2: AUC={m['auc']:.4f} AUPRC={m['auprc']:.4f} "
            f"fit={fit_sec / 3600:.2f}h "
            f"lambda_content_final={m.get('lambda_content_final')} "
            f"(v1.2 ndim32 ref combined=0.7745; v1.4 ref 0.7703)"
        )
        ...
        print(f"[pmp-v1.5] run_dir: {rl.run_dir}")
