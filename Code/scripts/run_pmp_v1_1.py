"""Run PMP v1.1 cold-start DDI experiment (rewrite 2026-06-02).

v1.1 replaces EmerGNN backbone scoring entirely with explicit pairwise cluster
path + per-cluster within-pool + final MLP score. Training protocol
shuffle_train(mode='S2') is inherited from EmerGNN.

Examples:
  # seed 42, single rep, default v1.1 settings
  python Code/scripts/run_pmp_v1_1.py --epochs 100 --tag pmp_v1_1_main_seed42

  # force rebuild both PMP cache and cluster cache
  python Code/scripts/run_pmp_v1_1.py --epochs 100 \
      --pmp-force-rebuild-cache --cluster-force-rebuild-cache \
      --tag pmp_v1_1_rebuild
"""
...
def _eval_s2(model, ds):
    ...
    # v1.1 reports 3 diagnostic branches via _predict_branches_v1_1:
    #   combined, cluster_only, within_only
    try:
        all_pairs = pd.concat([pos, neg], ignore_index=True)
        c, cluster_only, within_only = model._predict_branches_v1_1(all_pairs)
        ...
        print(f"[pmp-v1.1] per-branch diag failed: {exc}", flush=True)
...
def main():
    ...
    tag = args.tag or f"pmp_v1_1_seed{args.seed}"
    with RunLogger(script="run_pmp_v1_1", tag=tag, seed=args.seed) as rl:
        rl.set_meta(epochs=args.epochs, seed=args.seed)
        print(f"[pmp-v1.1] config: {vars(args)}")
        pkl_path = _pkl_for_seed(args.seed)
        print(f"[pmp-v1.1] using dataset: {pkl_path}")
        from data_utils import PairDataset
        ds = PairDataset.from_pkl(str(pkl_path))

        trainer = _PerModeEmerGNN_PMP_v1_1(
            n_dim=args.n_dim,
            length=args.length,
            feat="M",
            learning_rate=1e-3,
            ...
            # MNAH parent hyperparams (kept to satisfy fit() scaffold; v1.1
            # does NOT use the residual fusion path)
            mnah_hidden=32,
            mnah_dropout=0.2,
            mnah_init_beta=0.0,  # softplus(-5) ≈ 0; v1 PMP residual effectively off
            mnah_text_cache=None,
            # v1 PMP (cache loaded for mediator vocab + n1/n2)
        ...
        print(f"[pmp-v1.1] run_dir: {rl.run_dir}")
