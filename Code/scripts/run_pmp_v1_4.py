"""Run PMP v1.4 cold-start DDI experiment (created 2026-06-03).

v1.4 = Layer-2-only (per-cluster within pool) ablation of v1.2. The cluster
path is dropped entirely; score is computed from the within-pool alone. Used
to measure Layer 2's standalone cold-start AUC when trained in isolation.

Reuses PMP cache v1 + v1.1 cluster cache v2.

Example:
  python Code/scripts/run_pmp_v1_4.py --epochs 100 --seed 42 --n-dim 32 \
      --tag pmp_v1_4_ndim32_seed42
"""
...
from my_code.models.pmp_v1.v1_4.pmp_v1_4_trainer import (  # noqa: E402
    _PerModeEmerGNN_PMP_v1_4, DEFAULT_CLUSTER_CACHE,
)
...
def main():
    ...
    tag = args.tag or f"pmp_v1_4_seed{args.seed}"
    with RunLogger(script="run_pmp_v1_4", tag=tag, seed=args.seed) as rl:
        rl.set_meta(epochs=args.epochs, seed=args.seed)
        print(f"[pmp-v1.4] config: {vars(args)}")
        pkl_path = _pkl_for_seed(args.seed)
        print(f"[pmp-v1.4] using dataset: {pkl_path}")
        from data_utils import PairDataset
        ds = PairDataset.from_pkl(str(pkl_path))

        trainer = _PerModeEmerGNN_PMP_v1_4(
            n_dim=args.n_dim,
            length=args.length,
            feat="M",
            learning_rate=1e-3,
        ...
        print(
            f"[pmp-v1.4] test_s2: AUC={m['auc']:.4f} AUPRC={m['auprc']:.4f} "
            f"fit={fit_sec / 3600:.2f}h "
            f"(v1.2 ndim32 ref combined=0.7745 within_only=0.7344; "
        ...
        print(f"[pmp-v1.4] run_dir: {rl.run_dir}")
