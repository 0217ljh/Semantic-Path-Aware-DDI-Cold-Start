"""Run PMP v1.6 cold-start DDI experiment (created 2026-06-04).

v1.6 = PyG MessagePassing equivalent of v1.5 A. Strict mathematical
equivalence claim: same forward output as v1.5 A (within numerical
precision) for any input and any shared learnable param values.

Reuses PMP cache v1 + v1.1 cluster cache v2.

Example:
  python Code/scripts/run_pmp_v1_6.py --epochs 100 --seed 42 --n-dim 32 \
      --tag pmp_v1_6_ndim32_seed42
"""
...
from my_code.models.pmp_v1.v1_6.pmp_v1_6_trainer import (  # noqa: E402
    _PerModeEmerGNN_PMP_v1_6, DEFAULT_CLUSTER_CACHE,
)
...
def main():
    ...
    tag = args.tag or f"pmp_v1_6_seed{args.seed}"
    with RunLogger(script="run_pmp_v1_6", tag=tag, seed=args.seed) as rl:
        rl.set_meta(epochs=args.epochs, seed=args.seed)
        print(f"[pmp-v1.6] config: {vars(args)}")
        pkl_path = _pkl_for_seed(args.seed)
        print(f"[pmp-v1.6] using dataset: {pkl_path}")
        from data_utils import PairDataset
        ds = PairDataset.from_pkl(str(pkl_path))

        trainer = _PerModeEmerGNN_PMP_v1_6(
            n_dim=args.n_dim,
            length=args.length,
            feat="M",
            learning_rate=1e-3,
        ...
        print(
            f"[pmp-v1.6] test_s2: AUC={m['auc']:.4f} AUPRC={m['auprc']:.4f} "
            f"fit={fit_sec / 3600:.2f}h "
            f"(v1.5 A ref AUC=0.7774, AUPRC=0.7943 avg of 2 runs)"
        )
        ...
        print(f"[pmp-v1.6] run_dir: {rl.run_dir}")
