"""Run PMP v1.3 cold-start DDI experiment (created 2026-06-03).

v1.3 = Layer-1-only (pair-conditional cluster path) ablation of v1.2. The
within-cluster pool is dropped entirely; score is computed from the cluster
path alone. Used to measure Layer 1's standalone cold-start AUC when trained
in isolation.

Reuses PMP cache v1 + v1.1 cluster cache v2.

Example:
  python Code/scripts/run_pmp_v1_3.py --epochs 100 --seed 42 --n-dim 32 \
      --tag pmp_v1_3_ndim32_seed42
"""
...
from my_code.models.pmp_v1.v1_3.pmp_v1_3_trainer import (  # noqa: E402
    _PerModeEmerGNN_PMP_v1_3, DEFAULT_CLUSTER_CACHE,
)
...
def _eval_s2(model, ds):
    ...
    # v1.3 has no diagnostic split; cluster_only equals combined.
...
def main():
    ...
    tag = args.tag or f"pmp_v1_3_seed{args.seed}"
    with RunLogger(script="run_pmp_v1_3", tag=tag, seed=args.seed) as rl:
        rl.set_meta(epochs=args.epochs, seed=args.seed)
        print(f"[pmp-v1.3] config: {vars(args)}")
        pkl_path = _pkl_for_seed(args.seed)
        print(f"[pmp-v1.3] using dataset: {pkl_path}")
        from data_utils import PairDataset
        ds = PairDataset.from_pkl(str(pkl_path))

        trainer = _PerModeEmerGNN_PMP_v1_3(
            n_dim=args.n_dim,
            length=args.length,
            feat="M",
            learning_rate=1e-3,
        ...
        print(
            f"[pmp-v1.3] test_s2: AUC={m['auc']:.4f} AUPRC={m['auprc']:.4f} "
            f"fit={fit_sec / 3600:.2f}h "
            f"(v1.2 ndim32 ref combined=0.7745 cluster_only=0.7016)"
        )
        ...
        print(f"[pmp-v1.3] run_dir: {rl.run_dir}")
